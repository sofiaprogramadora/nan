# This file is auto-generated from the current state of the database. Instead
# of editing this file, please use the migrations feature of Active Record to
# incrementally modify your database, and then regenerate this schema definition.
#
# Note that this schema.rb definition is the authoritative source for your
# database schema. If you need to create the application database on another
# system, you should be using db:schema:load, not running all the migrations
# from scratch. The latter is a flawed and unsustainable approach (the more migrations
# you'll amass, the slower it'll run and the greater likelihood for issues).
#
# It's strongly recommended that you check this file into your version control system.

ActiveRecord::Schema.define(version: 20171030140006) do

  # These are extensions that must be enabled in order to support this database
  enable_extension "plpgsql"

  create_table "middle_tables", force: :cascade do |t|
    t.bigint "user_id"
    t.bigint "project_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["project_id"], name: "index_middle_tables_on_project_id"
    t.index ["user_id"], name: "index_middle_tables_on_user_id"
  end

  create_table "projects", force: :cascade do |t|
    t.string "title"
    t.text "content"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.bigint "users_id"
    t.bigint "user_id"
    t.index ["user_id"], name: "index_projects_on_user_id"
    t.index ["users_id"], name: "index_projects_on_users_id"
  end

  create_table "user_projects", force: :cascade do |t|
    t.bigint "users_id"
    t.bigint "projects_id"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.index ["projects_id"], name: "index_user_projects_on_projects_id"
    t.index ["users_id"], name: "index_user_projects_on_users_id"
  end

  create_table "users", force: :cascade do |t|
    t.string "name"
    t.string "password"
    t.datetime "created_at", null: false
    t.datetime "updated_at", null: false
    t.bigint "projects_id"
    t.bigint "project_id"
    t.index ["project_id"], name: "index_users_on_project_id"
    t.index ["projects_id"], name: "index_users_on_projects_id"
  end

  add_foreign_key "middle_tables", "projects"
  add_foreign_key "middle_tables", "users"
  add_foreign_key "projects", "users"
  add_foreign_key "projects", "users", column: "users_id"
  add_foreign_key "user_projects", "projects", column: "projects_id"
  add_foreign_key "user_projects", "users", column: "users_id"
  add_foreign_key "users", "projects"
  add_foreign_key "users", "projects", column: "projects_id"
end
